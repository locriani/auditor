...
`empty` and `error` are meant never to be the same value — the `stderr` column and the `TRUNCATED` note exist so a partial read does not pass as a complete one. Collapsing them is Empty-Result Ambiguity, the first pattern in the list above, and v1.0.0 of this script committed it. **The guarantee is not complete.** Three probes still discard stderr in-command and can report `empty` on a tree they could not read, and the test suite does not yet pin the `error` or `output` statuses — deliberately breaking either leaves it green. Treat the status column as a lead, not a verdict.
