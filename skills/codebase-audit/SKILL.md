...
```sh
uv run --project ${CLAUDE_PLUGIN_ROOT} auditor-probe <target-dir> -o <bundle> \
...
`empty` and `error` are meant never to be the same value. Collapsing them is Empty-Result Ambiguity, the first pattern in the list above, and v1.0.0 of this tool committed it. The status column is still the collector's opinion; the output file is the fact. What the probe cannot promise is listed in [What auditor-probe does not guarantee](#what-auditor-probe-does-not-guarantee) — read it once before relying on any `empty`.
...
5. The probe's entry under [What auditor-probe does not guarantee](#what-auditor-probe-does-not-guarantee).
...
## What auditor-probe does not guarantee

Known limits at 2.0.0. Each is a place where a status can be wrong without the bundle saying so.
...
- **Every dependency-scanner row must contain a report** (`composer-audit` `advisories`, `npm-audit` `auditReportVersion`/`vulnerabilities`, `pip-audit` `dependencies`, `go-vulncheck` `SBOM`, `cargo-audit` `vulnerabilities`, `bundler-audit` `results`, trivy's `Results`), or it reads `error`. Most of these tools exit the same code for "found something" and "could not audit". Each refuses a lock that would give a wrong answer, and reads `n/a`: `cargo-audit` without a `Cargo.lock` (it would write one into the target and report on versions resolved that day), `composer audit` without a `composer.lock` (it would audit installed packages, not the lock), and `bundler-audit` on a `Gemfile.lock` with no `specs:` section (a garbage lock exits 0, reported clean).
- **Target-shipped ignore lists are neutralised.** `composer.json`'s `config.policy.advisories.ignore` hid all 14 advisories in a real run and has no command-line override, so composer audits a copy of `composer.json` and `composer.lock` with the `config` block removed. `.bundler-audit.yml` hid one advisory, so bundler-audit gets `--config` pointing at an empty file.
- **Manifests are read wherever they sit.** Every ecosystem is discovered through the whole tree (vendored and build directories pruned). Each directory holding a manifest is a project, with one exception: a directory that has no lockfile of its own and sits inside another project is treated as a workspace member covered by that project's lock. A nested project's rows are named `probe@rel/dir` (`npm-audit@tools/cli`, `swift-audit@App/App.xcodeproj/project.xcworkspace/xcshareddata/swiftpm`), and their files in `out/` flatten `/` to `__`. The workspace rule is a heuristic. A nested manifest with no lock that is *not* a workspace member (a standalone example, say) goes unaudited, and no row says so.
...
- **Swift pins are scanned by trivy, never resolved.** `swift-audit` (`Package.resolved`) and `cocoapods-audit` (`Podfile.lock`) run `trivy fs` against the GitHub Advisory Database. trivy reads `trivy.yaml` and `.trivyignore` from its working directory, and a target-shipped copy hid every finding in a real run, so it runs from the bundle directory. A malformed lockfile still exits 0 with a report that has no `Results`; that reads `error`. GHSA keys Swift advisories by repository URL, so trivy cannot tell a vulnerable submodule from a clean one in the same repository. The probe never runs SwiftPM, because resolving compiles and executes `Package.swift`; `Package.swift` with no `Package.resolved` reads `n/a`. An Xcode project's `Package.resolved` under `*.xcodeproj/` is found like any nested manifest.
- **`cargo-licenses` is the `license` field each third-party crate declares**, from `cargo metadata --locked`; it never runs without a `Cargo.lock`. `NO LICENSE DECLARED` and `license-file:` rows need the file read. The field is the author's claim, not a license scan of the source.
- **`rust-markers` and `swift-markers` are counts, not findings.** They count each language's escape hatches (`unsafe`, `.unwrap()`, `let _ =`, `try!`, `try?`, `as!`, `@unchecked Sendable` and so on) outside paths containing `test`, to show where to read. Rust's inline `#[cfg(test)]` modules still count. Swift's postfix force-unwrap `!` is not counted.
