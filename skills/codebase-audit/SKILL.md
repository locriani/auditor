...
- **`npm-audit`, `pip-audit`, `cargo-audit`, `swift-audit` and `cocoapods-audit` rows must contain a report** (`auditReportVersion`/`vulnerabilities`, `dependencies`, `vulnerabilities`, or trivy's `Results`). `cargo-audit` does not run without a `Cargo.lock`: given none, it runs `cargo generate-lockfile`, writes a lockfile into the target, and reports clean against versions resolved that day. `composer audit`, `govulncheck` and `bundler-audit` accept their "found something" exit without a content check, so a failure with that exit and an error message reads `ok`. Open the file.
